...
export default function DataManagementTab() {
    ...
    const [selectedSubjectDept, setSelectedSubjectDept] = useState("Architectural");
    ...
    const loadAllSubjectTitles = async () => {
        try {
            ...
            if (res.ok) {
                ...
                const mock = {
                    Architectural: ["SUN SCREEN FOR A6 PARK-D", "FACADE DETAILS", "WINDOW SCHEDULE"],
                ...
                setEditingSubjects({ department: "Architectural", subjects: mock.Architectural });
            }
        } catch (error) {
    ...
    const handleRemoveSDElement = (index) => {
        const updated = editingSDDescriptions.elements.filter((_, i) => i !== index);
        ...
        try {
            const requestData = {
                ...
                descriptions: {
                    ...
                    elements: editingSDDescriptions.elements.filter(item => item.trim() !== ""),
                }
            };
            ...
            if (res.ok) {
                showToast(`SD descriptions for ${editingSDDescriptions.department} saved`, "success");
                await loadAllDescriptions();
            } else {
                const errorData = await res.json();
    ...
    const handleDeleteSDDepartment = async () => {
        if (!window.confirm(`Are you sure you want to delete all SD descriptions for "${editingSDDescriptions.department}"?\nThis action cannot be undone.`)) {
            return;
        }
        setSaving(true);
        try {
            const res = await fetch(`${API_URL}/admin/general-descriptions-sd/${editingSDDescriptions.department}`, {
                method: "DELETE"
            });
            if (res.ok) {
                showToast(`SD descriptions for ${editingSDDescriptions.department} deleted`, "success");
                setEditingSDDescriptions(prev => ({ ...prev, base: [], elements: [] }));
                await loadAllDescriptions();
            }
        } catch (error) {
    ...
    const handleAddSubject = () => {
        ...
        const updated = { ...editingSubjects, subjects: [...editingSubjects.subjects, newSubject.trim()] };
    ...
    const handleRemoveSubject = (index) => {
        const updated = editingSubjects.subjects.filter((_, i) => i !== index);
        setEditingSubjects(prev => ({ ...prev, subjects: updated }));
        showToast("Subject removed", "info");
    };

    const handleUpdateSubject = (index, newValue) => {
        const updated = [...editingSubjects.subjects];
        updated[index] = newValue;
        setEditingSubjects(prev => ({ ...prev, subjects: updated }));
    };
    ...
    const handleSaveSubjects = async () => {
        ...
        try {
            ...
            if (res.ok) {
                showToast(`Subject titles for ${editingSubjects.department} saved`, "success");
    ...
    const handleDeleteDepartmentSubjects = async () => {
        if (!window.confirm(`Are you sure you want to delete all subjects for "${editingSubjects.department}"?\nThis action cannot be undone.`)) {
            return;
        }
        setSaving(true);
        try {
            const res = await fetch(`${API_URL}/admin/subject-titles/${editingSubjects.department}`, {
                method: "DELETE"
            });
            if (res.ok) {
                showToast(`Subjects for ${editingSubjects.department} deleted`, "success");
    ...
    const exportSubjects = () => {
        ...
        const linkElement = document.createElement('a');
        linkElement.setAttribute('href', dataUri);
        linkElement.setAttribute('download', `subject-titles-${new Date().toISOString().split('T')[0]}.json`);
        linkElement.click();
    ...
    return (
        <div className="p-6">
            ...
            <div className="mb-8">
                ...
                <p className="text-gray-600">Manage location rules, general descriptions, sheet titles, and subject titles</p>
            </div>

            <div className="flex border-b border-gray-200 mb-6 overflow-x-auto">
                <button onClick={() => setActiveSection("location_rules")} className={`px-4 py-2 font-medium text-sm transition-colors ${activeSection === "location_rules" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-600 hover:text-gray-800"}`}>📍 Location Rules</button>
                <button onClick={() => setActiveSection("descriptions")} className={`px-4 py-2 font-medium text-sm transition-colors ${activeSection === "descriptions" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-600 hover:text-gray-800"}`}>📝 General Descriptions</button>
                <button onClick={() => setActiveSection("sheets")} className={`px-4 py-2 font-medium text-sm transition-colors ${activeSection === "sheets" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-600 hover:text-gray-800"}`}>📄 Sheet Titles</button>
                <button onClick={() => setActiveSection("subjects")} className={`px-4 py-2 font-medium text-sm transition-colors ${activeSection === "subjects" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-600 hover:text-gray-800"}`}>📌 Subject Titles</button>
                <button onClick={() => setActiveSection("tools")} className={`px-4 py-2 font-medium text-sm transition-colors ${activeSection === "tools" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-600 hover:text-gray-800"}`}>🛠️ Data Tools</button>
            </div>
            ...
            <div className="space-y-6">
                                    ...
                                    {Object.entries(projects).map(([projectName, projectData]) => {
                                        ...
                                        return (
                                            <div key={projectName} className={`border rounded-lg p-4 hover:shadow-md transition-shadow ${editingProjectRules.project === projectName ? "border-blue-500 bg-blue-50" : "border-gray-200"}`}>
                                                ...
                                                <div className="text-sm text-gray-600">Rules: {rules.length}</div>
                                            </div>
                                        );
                                    })}
                                </div>
                ...
                {activeSection === "descriptions" && (
                    <div className="space-y-6">
                        {/* Type selector */}
                        <div className="bg-white rounded-xl border border-gray-200 p-5">
                            <h3 className="text-lg font-bold text-gray-800 mb-4">📝 Description Type</h3>
                        ...
                        {descriptionType === "regular" && (
                            <div className="bg-white rounded-xl border border-gray-200 p-5">
                                <h3 className="text-lg font-bold text-gray-800 mb-4">🏗️ Select Department (Regular)</h3>
                                ...
                                <div className="flex items-center justify-between">
                                    <p className="text-sm text-gray-600">Department: <span className="font-medium">{selectedDepartment}</span></p>
                                    <div className="flex gap-2">
                        ...
                        {descriptionType === "cpr" && (
                            <div className="bg-white rounded-xl border border-gray-200 p-5">
                                <h3 className="text-lg font-bold text-gray-800 mb-4">🏗️ CPR Descriptions (Civil only)</h3>
                                <div className="flex items-center justify-between mb-6">
                                    <p className="text-sm text-gray-600">Department: <span className="font-medium">Civil</span></p>
                                    <div className="flex gap-2">
                                        <button onClick={handleDeleteDepartmentDescriptions} disabled={saving || (!editingDescriptions.base.length && !editingDescriptions.floors.length)} className="px-4 py-2 text-sm bg-red-100 hover:bg-red-200 text-red-700 rounded-lg disabled:opacity-50">🗑️ Delete All</button>
                                        <button onClick={handleSaveDescriptions} disabled={saving} className="px-4 py-2 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded-lg disabled:opacity-50">{saving ? "Saving..." : "💾 Save"}</button>
                                    </div>
                        ...
                        {descriptionType === "sd" && (
                            <div className="bg-white rounded-xl border border-gray-200 p-5">
                                <h3 className="text-lg font-bold text-gray-800 mb-4">📐 Select Department (SD)</h3>
                                <div className="flex flex-wrap gap-2 mb-6">
                                    {departments.map((dept) => (
                                        <button
                                            key={dept.value}
                                ...
                                <div className="flex items-center justify-between">
                                    <p className="text-sm text-gray-600">Department: <span className="font-medium">{editingSDDescriptions.department}</span></p>
                                    <div className="flex gap-2">
                                        <button onClick={handleDeleteSDDepartment} disabled={saving || (!editingSDDescriptions.base.length && !editingSDDescriptions.elements.length)} className="px-4 py-2 text-sm bg-red-100 hover:bg-red-200 text-red-700 rounded-lg disabled:opacity-50">🗑️ Delete All</button>
                                        <button onClick={handleSaveSDDescriptions} disabled={saving} className="px-4 py-2 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded-lg disabled:opacity-50">{saving ? "Saving..." : "💾 Save"}</button>
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        {descriptionType === "sd" ? (
                            <>
                                ...
                                <div className="bg-white rounded-xl border border-gray-200 p-5">
                                    <h4 className="font-medium text-gray-700 mb-4">📝 Base Descriptions ({editingSDDescriptions.base.length})</h4>
                                    {editingSDDescriptions.base.length === 0 ? (
                                        <div className="text-center py-6 text-gray-500 border-2 border-dashed border-gray-300 rounded-lg">No base descriptions</div>
                                    ) : (
                                        <div className="space-y-3">
                                            {editingSDDescriptions.base.map((desc, index) => (
                                                <div key={index} className="flex items-center gap-3 p-3 border rounded-lg hover:bg-gray-50">
                                                    ...
                                                    <input
                                                        type="text"
                                                        value={desc}
                                                        onChange={(e) => handleUpdateSDBase(index, e.target.value)}
                                                        className="flex-1 px-3 py-1 border rounded"
                                                    />
                                                    <button onClick={() => handleRemoveSDBase(index)} className="p-2 text-red-600 hover:text-red-800 hover:bg-red-100 rounded">🗑️</button>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                                ...
                                {/* SD Elements */}
                                <div className="bg-white rounded-xl border border-gray-200 p-5">
                                    <h4 className="font-medium text-gray-700 mb-4">🔧 Elements ({editingSDDescriptions.elements.length})</h4>
                                    {editingSDDescriptions.elements.length === 0 ? (
                                        <div className="text-center py-6 text-gray-500 border-2 border-dashed border-gray-300 rounded-lg">No elements</div>
                                    ) : (
                                        <div className="space-y-3">
                                            {editingSDDescriptions.elements.map((elem, index) => (
                                                <div key={index} className="flex items-center gap-3 p-3 border rounded-lg hover:bg-gray-50">
                                                    ...
                                                    <input
                                                        type="text"
                                                        value={elem}
                                                        onChange={(e) => handleUpdateSDElement(index, e.target.value)}
                                                        className="flex-1 px-3 py-1 border rounded"
                                                    />
                                                    <button onClick={() => handleRemoveSDElement(index)} className="p-2 text-red-600 hover:text-red-800 hover:bg-red-100 rounded">🗑️</button>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                ...
                {activeSection === "subjects" && (
                    <div className="space-y-6">
                        <div className="bg-white rounded-xl border border-gray-200 p-5">
                            <h3 className="text-lg font-bold text-gray-800 mb-4">📌 Subject Titles for Shop Drawing</h3>
                            <div className="flex flex-wrap gap-2 mb-6">
                                {departments.map(dept => (
                                    <button key={dept.value} onClick={() => handleSelectSubjectDept(dept.value)} className={`px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2 ${selectedSubjectDept === dept.value ? "bg-indigo-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>
                                        <span>{dept.icon}</span> {dept.label}
                                    </button>
                                ))}
                            </div>
                            <div className="flex items-center justify-between">
                                <p className="text-sm text-gray-600">Department: <span className="font-medium">{selectedSubjectDept}</span></p>
                                <div className="flex gap-2">
                                    <button onClick={handleDeleteDepartmentSubjects} disabled={saving || editingSubjects.subjects.length === 0} className="px-4 py-2 text-sm bg-red-100 hover:bg-red-200 text-red-700 rounded-lg disabled:opacity-50">🗑️ Delete All</button>
                                    <button onClick={handleSaveSubjects} disabled={saving} className="px-4 py-2 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded-lg disabled:opacity-50">{saving ? "Saving..." : "💾 Save"}</button>
                                </div>
                            </div>
                        </div>

                        <div className="bg-white rounded-xl border border-gray-200 p-5">
                            <h4 className="font-medium text-gray-700 mb-4">➕ Add New Subject</h4>
                            <div className="flex gap-2">
                                <input type="text" value={newSubject} onChange={(e) => setNewSubject(e.target.value)} className="flex-1 px-3 py-2 border rounded-lg" placeholder="Enter subject (e.g., SUN SCREEN FOR A6 PARK-D)" />
                                <button onClick={handleAddSubject} disabled={!newSubject.trim()} className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg disabled:opacity-50">Add</button>
                            </div>
                        </div>

                        <div className="bg-white rounded-xl border border-gray-200 p-5">
                            <h4 className="font-medium text-gray-700 mb-4">📋 Subject Titles ({editingSubjects.subjects.length})</h4>
                            {editingSubjects.subjects.length === 0 ? (
                                <div className="text-center py-6 text-gray-500 border-2 border-dashed border-gray-300 rounded-lg">No subjects</div>
                            ) : (
                                <div className="space-y-3">
                                    {editingSubjects.subjects.map((subject, index) => (
                                        <div key={index} className="flex items-center gap-3 p-3 border rounded-lg hover:bg-gray-50">
                                            <span className="text-gray-500">{index + 1}.</span>
                                            <input type="text" value={subject} onChange={(e) => handleUpdateSubject(index, e.target.value)} className="flex-1 px-3 py-1 border rounded" />
                                            <button onClick={() => handleRemoveSubject(index)} className="p-2 text-red-600 hover:text-red-800 hover:bg-red-100 rounded">🗑️</button>
                                        </div>
                                    ))}
                                </div>
                            )}
                        ...
                        <div className="bg-white rounded-xl border border-gray-200 p-5">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="border rounded-lg p-4">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <button onClick={exportSubjects} className="w-full px-4 py-3 bg-indigo-100 hover:bg-indigo-200 text-indigo-700 rounded-lg font-medium flex items-center justify-between">Export Subject Titles <span>⬇️</span></button>
                                        <button onClick={() => {
                                            const allData = {
                                ...
                                <div className="border rounded-lg p-4">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <label className="block">
                                            <div className="px-4 py-3 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-lg font-medium flex items-center justify-between cursor-pointer">Import Subject Titles <span>⬆️</span></div>
                                            <input type="file" accept=".json" onChange={(e) => handleImportData(e, "subjects")} className="hidden" />
                                        </label>
                            ...
                            <div className="mt-8 pt-6 border-t border-gray-200">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                                    ...
                                    <div className="text-center p-3 bg-gray-50 rounded-lg">
                                        <div className="text-2xl font-bold text-purple-600">{Object.keys(sdDescriptions).length}</div>
                                        <div className="text-sm text-gray-600">Depts (SD)</div>
                                    </div>
                                    <div className="text-center p-3 bg-gray-50 rounded-lg">
                                        <div className="text-2xl font-bold text-indigo-600">{Object.keys(sheetTitles).length}</div>
