...
export default function EngineerRecords() {
  ...
  const loadDepartmentRecords = useCallback(async () => {
    ...
    setError("");
    try {
      // get all IRs from main collection
      ...
      const departmentIRs = allIRs.filter(
        (ir) =>
          ir.department &&
          getDepartmentAbbr(ir.department) ===
            getDepartmentAbbr(userDepartment),
      );

      console.log("irs Res:", irsRes);
      console.log("IRS DATA:", irsData);
      console.log("ALL IRSSSS:", allIRs);
      console.log(
        `IRSDEPARTMENTS ${departmentIRs.length} IRs for department ${userDepartment}`,
      );

      // get all Revisions from main collection
      const revsRes = await fetch(`${API_URL}/revs`);
      if (!revsRes.ok)
        throw new Error(`Failed to load Revisions: ${revsRes.status}`);

      const revsData = await revsRes.json();
      const allRevisions = revsData.revs || [];
      const departmentRevisions = allRevisions.filter(
        (rev) =>
          rev.department &&
      ...
      if (!archiveIrsRes.ok)
        throw new Error(`Failed to load archive IRs: ${archiveIrsRes.status}`);

      const archiveIrsData = await archiveIrsRes.json();
      const allArchiveIRs = archiveIrsData.archive || [];
      const departmentArchiveIRs = allArchiveIRs.filter(
        (item) =>
          !item.isRevision &&
          item.department &&
          getDepartmentAbbr(item.department) ===
            getDepartmentAbbr(userDepartment),
      );
      ...
      if (!archiveRevsRes.ok)
        throw new Error(
          `Failed to load archive Revisions: ${archiveRevsRes.status}`,
        );

      const archiveRevsData = await archiveRevsRes.json();
      const allArchiveRevs = archiveRevsData.archive || [];
      const departmentArchiveRevs = allArchiveRevs.filter(
        (item) =>
          item.isRevision &&
          item.department &&
          getDepartmentAbbr(item.department) ===
      ...
      const allRecords = [
        ...departmentIRs.map((ir) => ({
          ...
          irAttatch: ir.tags.engineer || "",
          sdAttatch: ir.tags.sd || "",
          itemType: "IR",
      ...
      setToast(
        `✅ Loaded ${uniqueRecords.length} records from ${userDepartment} department (${departmentArchiveIRs.length + departmentArchiveRevs.length} archived)`,
      );
    } catch (err) {
  ...
  const stats = {
    ...
    cpr: records.filter(
      (r) => !r.isRevision && (r.requestType === "CPR" || r.isCPR),
    ).length,
    ir: records.filter(
      (r) => !r.isRevision && r.requestType !== "CPR" && !r.isCPR,
    ).length,
  ...
  const handleArchive = useCallback(
    async (item) => {
      if (
        !window.confirm(
          `Archive ${item.displayNumber}?\n\nThis will move the item to archive.`,
        )
      )
        return;
  ...
  const handleRestore = useCallback(
    async (item) => {
      if (!window.confirm(`Restore ${item.displayNumber} from archive?`))
        return;
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <h2 className="text-2xl font-bold mb-2">
                {userDepartment} Department Records
              </h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 text-gray-700 border-b">
                ...
                <th className="p-4 text-left font-semibold">ATT</th>
            ...
            <tbody>
              {filteredRecords.map((item, index) => (
                ...
                >
                  ...
                  <td className="p-4">
                    <span
                      className={`px-3 py-1.5 rounded-full text-xs font-medium ${getTypeColor(item)}`}
                    >
                  ...
                  <td className="p-4">
                    <div className="space-y-2">
                      <span
                        className={`px-3 py-1.5 rounded-full text-xs font-medium flex items-center gap-1 w-fit ${getStatusColor(item)}`}
                      >
                        {item.isArchived ? (
                          <>
                            <span>📁</span> Archived
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white/50 p-3 rounded-lg">
                  ...
                  <p className="text-blue-600 text-xs mt-1">
                    • View all records from your department
                    <br />
                    • See requests from all users in department
                    <br />
                    • Filter by user, project, type, and status
                    <br />• Track department performance
                  </p>
                </div>
                <div className="bg-white/50 p-3 rounded-lg">
                  ...
                  <p className="text-emerald-600 text-xs mt-1">
                    ...
                    <br />• Pending: {stats.pending}, Completed:{" "}
                    {stats.completed}
                    <br />• IRs: {stats.ir}, CPRs: {stats.cpr}, Revisions:{" "}
                    {stats.revisions}
                    <br />• Users in department: {stats.users}
                  </p>
                </div>
                <div className="bg-white/50 p-3 rounded-lg">
                  ...
                  <p className="text-purple-600 text-xs mt-1">
                    ...
                    <br />• Restore archived items when needed
                  </p>
                </div>
              </div>
            </div>
